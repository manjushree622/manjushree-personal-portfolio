...
export default function Page() {
  ...
  return <main className="site-shell">
    ...
    <section id="certifications" className="section-pad section-dark"><SectionHeading eyebrow="05 / CERTIFICATIONS" title="Milestones along the way."/><div className="cert-grid">{certifications.map(([title,org],i)=><div className="cert-card reveal" key={title}><div className="cert-icon"><Trophy size={18}/></div><div><span className="cert-org">{org}</span><h3>{title}</h3>{(['IBM Certificate in Python', 'Wadhwani Foundation Certificate', 'Instagram Design System Online Course Certificate'].includes(title)) && <button className="certificate-view-button" type="button" onClick={() => setActiveCertificate(title === 'IBM Certificate in Python' ? { title: 'Certificate in Python', image: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/ibm1-iq4f7sV9i9VxbW1Jmb003McpSq1xWG.jpeg', alt: 'IBM SkillsBuild certificate awarded to Manjushree for Data Analysis with Python' } : title === 'Wadhwani Foundation Certificate' ? { title: 'Wadhwani Foundation Certificate', image: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Wadhwani-wG9r0VhsMxIXOmVXuRZSeIfAULXp7V.jpeg', alt: 'Wadhwani Foundation certificate awarded to Manjushree' } : { title: 'Instagram Design System Course Certificate', image: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-6kPwCiMWgW3NALWmvsSMjUKkZNFWMA.png', alt: 'Instagram Design System course certificate awarded to Manjushree' })}>View certificate <ArrowUpRight size={14}/></button>}</div><span className="cert-index">0{i+1}</span></div>)}</div><div className="learning-card reveal"><div className="learning-icon"><Sparkles size={21}/></div><div><span className="eyebrow">CURRENTLY LEARNING</span><h3>Studio Course</h3><p>Currently exploring a Studio course to strengthen my technical knowledge and practical development skills.</p></div><span className="progress-badge"><span/> In Progress</span></div></section>
